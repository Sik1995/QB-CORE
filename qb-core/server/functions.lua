...
function QBCore.Functions.ExecuteSql(wait, query, cb)
	local rtndata = {}
	local waiting = true
...
-- New Notify Trigger
function QBCore.Functions.Notify(src, text, texttype, length)
    TriggerClientEvent('QBCore:Notify', src, text, texttype, length)
end