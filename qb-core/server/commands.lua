...
QBCore.Commands.Add('removepermission', 'Remove Players Permissions (God Only)', { { name = 'id', help = 'ID of player' } }, true, function(source, args)
    ...
    if Player then
        ...
        exports['ws-log']:Log('permission', Player.PlayerData.source, 'no longer `%s` by %s.', permission, Player.PlayerData.source == source and 'himself' or exports['ws-log']:FetchFormatedIds(source));
    else
        TriggerClientEvent('QBCore:Notify', src, 'Player Not Online', 'error')
    end
end, 'god')
